server <- function(input, output){
  ...
  output$word_count <- renderPlotly({
    ...
    if(input$text=="Words"){
      phrase <- texts
      colnames(phrase)[1] <- "phrase"
      phrase <- phrase[order(phrase$Freq, decreasing = T),]
      phrase$Prop <- round(phrase$Prop, 4)*100
    } else if(input$text=="Noun-Noun Phrase") {
      phrase <- strsplit(omnibus$anno_nNPhrase, "; ")
      phrase <- unlist(phrase)
      phrase <- as.data.frame(table(phrase))
      phrase <- phrase[order(phrase$Freq, decreasing = T),]
      phrase$Prop <- phrase$Freq/sum(phrase$Freq)
      phrase$Prop <- round(phrase$Prop, 4)*100
    } else {
      phrase <- strsplit(omnibus$anno_nAdjPhrase, "; ")
      phrase <- unlist(phrase)
      phrase <- as.data.frame(table(phrase))
      phrase <- phrase[order(phrase$Freq, decreasing = T),]
      phrase$Prop <- phrase$Freq/sum(phrase$Freq)
    ...
    p5 <- ggplot(head(phrase, 20), aes(reorder(phrase, Freq), Freq, 
                                      text = paste(Freq, " (", Prop, "%)", sep = ""))) + 
      geom_col(fill="skyblue") + theme_minimal() + coord_flip() + chart_theme + 
      labs(title = paste("Top 20 Most Frequent",input$text), x = "Terms", y = "Frequency")
  ...
  output$word_cloud <- renderWordcloud2({
    ...
    if(input$text=="Words"){
      text <- VCorpus(VectorSource(omnibus$text_cleaned))
      dtm <- as.matrix(DocumentTermMatrix(text))
      texts <- data.frame(Terms = colnames(dtm), Freq = colSums(dtm), row.names = NULL)
      colnames(texts)[1] <- "texts"
      texts <- texts[order(texts$Freq, decreasing = T),]
      wordcloud2(head(texts,100), shuffle = F)
    } else if(input$text=="Noun-Noun Phrase") {
      texts <- strsplit(omnibus$anno_nNPhrase, "; ")
      texts <- unlist(texts)
      texts <- as.data.frame(table(texts))
      texts <- texts[order(texts$Freq, decreasing = T),]
      wordcloud2(head(texts,30), shuffle = F, size = .7)
    } else {
      texts <- strsplit(omnibus$anno_nAdjPhrase, "; ")
      texts <- unlist(texts)
      texts <- as.data.frame(table(texts))
      texts <- texts[order(texts$Freq, decreasing = T),]
      wordcloud2(head(texts,30), shuffle = F, size = .3)
    }
  ...
  # valueBox(paste(round(score[2,2],2), "%", sep = ""), "Positive", color = "green", icon = icon("twitter"))
  # valueBox(paste(round(score[2,1],2), "%", sep = ""), "Negative", color = "green", icon = icon("twitter"))
  ...
  output$dt <- renderDataTable({
    ...
    datatable(omnibus,
              colnames = c("Username", "Gender", "Followers", "Post Date", "Tweet", "Cleaned Tweet", 
                           "Polarity", "Word Count", "Source App", "Noun-Adjective Phrase", 
                           "Noun-Noun Phrase", "Relevance Score"),
              options = list(scrollY=T, autoWidth=T, scrollX=T), filter = "top")
